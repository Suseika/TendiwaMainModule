...
public void draw() {
	...
	EnhancedRectangle rightTop = crs.addRectangle(new EnhancedRectangle(x, y, width, height));
	EnhancedRectangle rightBottom = crs.cutRectangleFromSide(rightTop, frontSide, rightTop.getDimensionBySide(frontSide) / 2);
	EnhancedRectangle leftTop = crs.cutRectangleFromSide(rightTop, leftSide, rightTop.getDimensionBySide(leftSide) / 2);
	EnhancedRectangle leftBottom = crs.cutRectangleFromSide(rightBottom, leftSide, rightBottom.getDimensionBySide(leftSide) / 2);
	...
	if (centralPlaceSizeMod > 0) {
		...
		if (recCenter.width == 1) {
			...
			if (chance < 20) {
				...
				settlement.setObject(recCenter.x + 1, recCenter.y + 1, objStatueDefender1);
				settlement.square(recCenter, FloorTypes.water, false);
			} else if (chance < 60) {
			...
			settlement.square(recCenter, FloorTypes.water, true);
		...
		Coordinate c8 = leftBottom
				...
				.moveToSide(rightSide, 1)
				.moveToSide(frontSide, 1);
		settlement.line(c1.x, c1.y, c2.x, c2.y, FloorTypes.stone);
		settlement.line(c3.x, c3.y, c4.x, c4.y, FloorTypes.stone);
		settlement.line(c5.x, c5.y, c6.x, c6.y, FloorTypes.stone);
		settlement.line(c7.x, c7.y, c8.x, c8.y, FloorTypes.stone);
